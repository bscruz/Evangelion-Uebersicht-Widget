...
#   Define constants
    ...
    if (language.indexOf("de") > -1)
        ...
        $(domEl).find("#21     .contentS").html("<u></u><d></d>#{window.Love}") # was window.Warning Message changed to LOVe
#   Voice
...
# Execution
    ...
    @run "sh Eva.widget/iTunes.sh 2>/dev/null", (error, stdout, stderr) ->
        ...
        if (stdout.indexOf("¶ ¶ ¶ 0 ¶ 0") > -1)
            ...
            $(domEl).find(".CoverCell").css("visibility","hidden") # changed all of them to hideen because i dont want to see no cover cell
            $(domEl).find("#44").css("visibility","hidden")
            $(domEl).find("#45").css("visibility","hidden")
            $(domEl).find("#48").css("visibility","hidden")
